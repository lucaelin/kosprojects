...
print "Making orbit polar.".
print "Coasting to equator.".
if (ABS(ORBIT:INCLINATION)-90 < 0) = (SHIP:GEOPOSITION:LAT < 0) {
  lock STEERING to vcrs(ship:velocity:orbit,-body:position). // NORMAL (on ascending node)
} else {
  lock STEERING to vcrs(ship:velocity:orbit,body:position). // ANTINORMAL (on descending node)
}
if SHIP:GEOPOSITION:LAT > 0 {
  wait until SHIP:GEOPOSITION:LAT < 0.
} else {
...
lock tgt to ABS(ABS(ORBIT:INCLINATION)-90).
